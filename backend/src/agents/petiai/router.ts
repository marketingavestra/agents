...
petiaRouter.post('/run', async (req, res) => {
  ...
  try {
    ...
    const conversationId = await ensureConversation(userId, 'petiai');
    ...
    await db.query(
      ...
      [uuidv4(), DEFAULT_TENANT_ID, conversationId, 'in', `web:${userKey}`, 'agent:petiai', query]
    );

    const result = await runPetiAI(query, history);

    await db.query(
      'INSERT INTO messages (id, tenant_id, conversation_id, direction, from_jid, to_jid, body) VALUES ($1,$2,$3,$4,$5,$6,$7)',
      [uuidv4(), DEFAULT_TENANT_ID, conversationId, 'out', 'agent:petiai', `web:${userKey}`, result.text]
    );
    ...
    await logRun({ agent: 'petiai', status: 'OK', durationMs: Date.now() - started, tokens: totalTokens, costCents: 0, extra: result.usage });
...
petiaRouter.post('/clear', async (req, res) => {
  const userKey = (req.headers['x-user'] as string) || req.body?.user_key || 'web-anon';
  const userEmail = userKey.includes('@') ? userKey : `${userKey}@local`;
  try {
    ...
    const c = await db.query(
      ...
      [DEFAULT_TENANT_ID, userId, 'petiai']
    );
    if (c.rowCount > 0) {
