...
contrataiRouter.post('/run', async (req, res) => {
  ...
  try {
    ...
    const conversationId = await ensureConversation(userId, 'contratai');
    ...
    await db.query(
      ...
      [uuidv4(), DEFAULT_TENANT_ID, conversationId, 'in', `web:${userKey}`, 'agent:contratai', query]
    );

    const result = await runContratAI(query, history);

    await db.query(
      'INSERT INTO messages (id, tenant_id, conversation_id, direction, from_jid, to_jid, body) VALUES ($1,$2,$3,$4,$5,$6,$7)',
      [uuidv4(), DEFAULT_TENANT_ID, conversationId, 'out', 'agent:contratai', `web:${userKey}`, result.text]
    );
    ...
    await logRun({ agent: 'contratai', status: 'OK', durationMs: Date.now() - started, tokens: totalTokens, costCents: 0, extra: result.usage });
...
contrataiRouter.post('/clear', async (req, res) => {
  const userKey = (req.headers['x-user'] as string) || req.body?.user_key || 'web-anon';
  const userEmail = userKey.includes('@') ? userKey : `${userKey}@local`;
  try {
    ...
    const c = await db.query(
      ...
      [DEFAULT_TENANT_ID, userId, 'contratai']
    );
    if (c.rowCount > 0) {
